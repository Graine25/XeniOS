    ...
    if (!ui_capture_done) {
      ...
      if (capture_env && std::strcmp(capture_env, "1") == 0) {
        ...
        if (capture_manager) {
          ...
          [descriptor setDestination:MTLCaptureDestinationGPUTraceDocument];
          const char* capture_dir = std::getenv("XENIA_GPU_CAPTURE_DIR");
          std::string filename = capture_dir ? (std::string(capture_dir) + "/ui_capture.gputrace")
                                             : std::string("./ui_capture.gputrace");
          NSURL* output_url =
              [NSURL fileURLWithPath:[NSString stringWithUTF8String:filename.c_str()]];
          [descriptor setOutputURL:output_url];
          NSError* error = nil;
          if (![capture_manager startCaptureWithDescriptor:descriptor error:&error]) {
...
bool MetalPresenter::UpdateGammaRamp(const void* table_data, size_t table_bytes,
  ...
  if (::cvars::metal_presenter_log_gamma_ramp) {
    const uint32_t* table_words = reinterpret_cast<const uint32_t*>(contents);
    const uint32_t* pwl_words =
...
kernel void xe_copy_texture_convert_2d_array(
  ...
  // Release intermediate objects now that pipelines are created
  [function_2d_array release];
  [function_2d release];
  [library release];
...
bool MetalPresenter::EnsureApplyGammaDebugPipelines() {
  if (apply_gamma_debug_gradient_pipeline_ && apply_gamma_debug_copy_pipeline_ &&
      apply_gamma_debug_ramp_table_pipeline_ && apply_gamma_debug_ramp_pwl_pipeline_) {
    return true;
  }
...
kernel void xe_gamma_debug_ramp_pwl(
  ...
  id<MTLLibrary> library = [mtl_device newLibraryWithSource:source options:nil error:&error];
  if (!library) {
    XELOGE("MetalPresenter::EnsureApplyGammaDebugPipelines: Failed to compile "
           "library: {}",
           error ? error.localizedDescription.UTF8String : "unknown error");
    return false;
  }
  ...
  auto create_pipeline = [&](const char* name) -> id<MTLComputePipelineState> {
    ...
    if (!function) {
      XELOGE("MetalPresenter::EnsureApplyGammaDebugPipelines: Missing {} function", name);
      return nil;
    }
    id<MTLComputePipelineState> pipeline = [mtl_device newComputePipelineStateWithFunction:function
                                                                                     error:&error];
    if (!pipeline) {
      XELOGE("MetalPresenter::EnsureApplyGammaDebugPipelines: Failed to create {} "
             "pipeline: {}",
  ...
  if (!apply_gamma_debug_gradient_pipeline_) {
    apply_gamma_debug_gradient_pipeline_ = create_pipeline("xe_gamma_debug_gradient");
  }
  if (!apply_gamma_debug_copy_pipeline_) {
    apply_gamma_debug_copy_pipeline_ = create_pipeline("xe_gamma_debug_copy");
  }
  if (!apply_gamma_debug_ramp_table_pipeline_) {
    apply_gamma_debug_ramp_table_pipeline_ = create_pipeline("xe_gamma_debug_ramp_table");
  }
  if (!apply_gamma_debug_ramp_pwl_pipeline_) {
    apply_gamma_debug_ramp_pwl_pipeline_ = create_pipeline("xe_gamma_debug_ramp_pwl");
  }

  [library release];
  return apply_gamma_debug_gradient_pipeline_ && apply_gamma_debug_copy_pipeline_ &&
         apply_gamma_debug_ramp_table_pipeline_ && apply_gamma_debug_ramp_pwl_pipeline_;
}
...
bool MetalPresenter::CopyTextureToGuestOutput(MTL::Texture* source_texture, id dest_texture,
  ...
  if (gamma_debug_mode != 0) {
    ...
    if (!compute_encoder) {
      XELOGE("MetalPresenter::CopyTextureToGuestOutput: Failed to create compute "
    ...
    if (gamma_debug_mode == 1) {
      ...
      if (!gamma_ramp_table_texture_) {
        XELOGW("MetalPresenter::CopyTextureToGuestOutput: gamma debug mode 3 "
               "requires table ramp texture");
        [compute_encoder endEncoding];
        release_views();
        return false;
      }
      ramp_width = uint32_t([(id<MTLTexture>)gamma_ramp_table_texture_ width]);
    } else if (gamma_debug_mode == 4) {
      pipeline = apply_gamma_debug_ramp_pwl_pipeline_;
      if (!gamma_ramp_pwl_texture_) {
        XELOGW("MetalPresenter::CopyTextureToGuestOutput: gamma debug mode 4 "
               "requires PWL ramp texture");
        [compute_encoder endEncoding];
        release_views();
        return false;
      }
      ramp_width = uint32_t([(id<MTLTexture>)gamma_ramp_pwl_texture_ width]);
    } else {
      XELOGW("MetalPresenter::CopyTextureToGuestOutput: unknown gamma debug mode "
             "{}",
             gamma_debug_mode);
      [compute_encoder endEncoding];
      release_views();
